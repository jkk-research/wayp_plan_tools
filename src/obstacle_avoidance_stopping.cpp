...
class ObstacleAvoidanceTrapezoid : public rclcpp::Node
{
     rcl_interfaces::msg::SetParametersResult parametersCallback(const std::vector< rclcpp::Parameter > &parameters)
    {
        ...
        for (const auto &param : parameters)
        {
            ...
            else if (param.get_name() == "stop_before_obstacle")
            {
                stop_before_obstacle = param.as_double();
            }
            else if (param.get_name() == "deceleration")
            {
                deceleration = param.as_double();
            }
   ...
   public:
        ...
        : Node("obstacle_avoidance_trapezoid")
    {
        ...
        this->declare_parameter("stop_before_obstacle", stop_before_obstacle);
        this->declare_parameter("deceleration", deceleration);
        ...
        this->get_parameter("stop_before_obstacle", stop_before_obstacle);
        ...
        speed_sub_ = this->create_subscription<std_msgs::msg::Float32MultiArray>(speed_topic, 10, std::bind(&ObstacleAvoidanceTrapezoid::speed_callback, this, std::placeholders::_1));
...
public:
        ...
        if (waypoints_ != nullptr && current_pose_ != nullptr) 
        {
            ...
            else if (is_calculated && is_avoiding)
            {
                ...
                if (!is_trapezoid && start_index != -1 && end_index != -1)
                {
                    ...
                    calculateStopping(waypoints_, speed_, first_index, stop_before_obstacle, deceleration);
                    is_trapezoid = true;
    ...
    void calculateStopping(geometry_msgs::msg::PoseArray::SharedPtr waypoints_, std_msgs::msg::Float32MultiArray::SharedPtr speed_, int first_index, double stop_before_obstacle, double deceleration)
    {
        int stop_index = first_index - static_cast<int>(stop_before_obstacle);
        if (stop_index < 0) stop_index = 0;

        double distance_to_stop = 0.0;
        for (int i = stop_index; i < first_index; ++i)
        {
            distance_to_stop += distanceBetweenPoints(waypoints_->poses[i], waypoints_->poses[i + 1]);
        }

        double current_speed = speed_->data[stop_index];
        double required_deceleration_distance = (current_speed * current_speed) / (2 * deceleration);
        double accumulated_distance = 0.0;
        int deceleration_start_index = stop_index;

        for (int i = stop_index; i >= 0; --i)
        {
            accumulated_distance += distanceBetweenPoints(waypoints_->poses[i], waypoints_->poses[i + 1]);
            if (accumulated_distance >= required_deceleration_distance)
            {
                deceleration_start_index = i;
                break;
            }
        }

        double deceleration_distance = 0.0;
        for (int i = deceleration_start_index; i < stop_index; ++i)
        {
            double segment_distance = distanceBetweenPoints(waypoints_->poses[i], waypoints_->poses[i + 1]);
            deceleration_distance += segment_distance;
            double new_speed = current_speed - (deceleration * (deceleration_distance / required_deceleration_distance));
            if (new_speed < 0.0) new_speed = 0.0;
            speed_->data[i] = new_speed;
        }

        for (int i = stop_index; i < waypoints_size; ++i)
        {
            speed_->data[i] = 0.0;
        }

        deceleration_stop_index = stop_index;

        RCLCPP_INFO(this->get_logger(), "Deceleration start index: %d, Deceleration stop index: %d", deceleration_start_index, deceleration_stop_index);
        RCLCPP_INFO(this->get_logger(), "First index: %d", first_index);

        // Set speed to 0 until first_index
        for (int i = deceleration_stop_index; i < first_index; ++i)
        {
            speed_->data[i] = 0.0;
        }

        // // Print new waypoint speeds
        // for (int i = 0; i < waypoints_size; ++i)
        // {
        //     RCLCPP_INFO(this->get_logger(), "Waypoint %d new speed: %f", i, speed_->data[i]);
        // }
    }
    ...
    void publishDebugMarkers (size_t closest_waypoint_index, size_t start_index, size_t end_index ,size_t first_index,size_t last_index, geometry_msgs::msg::PoseArray::SharedPtr waypoints_,int lookahead_distance_index)
    {
        ...
        if (closest_waypoint_index != prev_closest_waypoint_index || start_index != prev_start_index || end_index != prev_end_index || first_index != prev_first_index || last_index != prev_last_index) 
        {
            ...
            if (waypoints_ != nullptr) 
            {
                ...
                if (deceleration_start_index != -1 && deceleration_stop_index != -1)
                {
                    // Create a Marker for the deceleration start waypoint
                    ...
                    deceleration_start_marker.id = 9;
                    ...
                    // Create a Marker for the deceleration stop waypoint
                    visualization_msgs::msg::Marker deceleration_stop_marker;
                    deceleration_stop_marker.header.frame_id = "map";
                    deceleration_stop_marker.header.stamp = this->now();
                    deceleration_stop_marker.ns = "waypoints";
                    deceleration_stop_marker.id = 10;
                    deceleration_stop_marker.type = visualization_msgs::msg::Marker::ARROW;
                    deceleration_stop_marker.action = visualization_msgs::msg::Marker::ADD;
                    deceleration_stop_marker.pose = waypoints_->poses[deceleration_stop_index];
                    deceleration_stop_marker.color.r = 0.0;
                    deceleration_stop_marker.color.g = 1.0;
                    deceleration_stop_marker.color.b = 1.0;
                    deceleration_stop_marker.color.a = 1.0;
                    deceleration_stop_marker.scale.x = 1.5;
                    deceleration_stop_marker.scale.y = 1.1;
                    deceleration_stop_marker.scale.z = 1.1;
                    debug_marker_array->markers.push_back(deceleration_stop_marker);
                }
