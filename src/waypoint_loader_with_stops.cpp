...
class WaypointLoaderWithStops : public rclcpp::Node
{
    rcl_interfaces::msg::SetParametersResult parametersCallback(const std::vector< rclcpp::Parameter > &parameters)
    {
        ...
        for (const auto &param : parameters)
        {
            ...
            if (param.get_name() == "stop_interval")
            {
   ...
   public:
    WaypointLoaderWithStops()
        : Node("waypoint_loader_w_stops")
    {
        ...
        */
        ...
        callback_handle_ = this->add_on_set_parameters_callback(std::bind(&WaypointLoaderWithStops::parametersCallback, this, std::placeholders::_1));
   ...
   private:
    ...
    void pubLaneWaypoint(const std::string &file_path)
    {
        ...
        for (std::vector< geometry_msgs::msg::Pose >::iterator it = wps_c.begin(); it != wps_c.end(); ++it)
        {
            ...
            float x_diff = it->position.x - prev_pose.position.x;
            float y_diff = it->position.y - prev_pose.position.y;
            sum_distance += std::sqrt(x_diff * x_diff + y_diff * y_diff);
            // if (id % 6 == 0)
            if (sum_distance > stop_interval_)
            {
                visualization_msgs::msg::Marker stop_elem;
                stop_elem.header.frame_id = "/map";
                stop_elem.header.stamp = this->now();
                stop_elem.ns = "stop";
                stop_elem.id = id;
                stop_elem.scale.x = 0.4;
                stop_elem.scale.y = 4.2;
                stop_elem.scale.z = 0.8;
                stop_elem.color.r = 0.72;  //  0.72 0.11 0.11 md_red_900 #B71C1C https://github.com/jkk-research/colors
                stop_elem.color.g = 0.11;
                stop_elem.color.b = 0.11;
                stop_elem.color.a = 0.8;
                stop_elem.type = visualization_msgs::msg::Marker::CUBE;
                stop_elem.action = visualization_msgs::msg::Marker::MODIFY;
                stop_elem.pose = *it;
    ...
    void
    reinitCallback(const std_msgs::msg::Bool &msg)
    {
        reinit = msg.data;
    }
...
int main(int argc, char **argv)
{
    ...
    rclcpp::spin(std::make_shared< WaypointLoaderWithStops >());
    rclcpp::shutdown();
    return 0;
}